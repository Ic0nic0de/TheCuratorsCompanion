...
Event OnSetup(string eventName, string strArg, float numArg, Form sender) ;;Runs Once, Automatic Call from (RN_Utility_Script)		
	...
	if !_setupDone
		...
		If MCM.DevDebugVal
			DBMDebug.Log(Self, "TCC: Setup Event Received for: CheeseMod")
		endIf
		...
		_onConsolidateDisplays(DBM_YUMCheeseDisplay, DBM_SectionSafehouse_Merged)
		...
		If MCM.DevDebugVal
			DBMDebug.Log(Self, "TCC: Setup Event Completed for: CheeseMod")
		endIf
	else
		...
		If MCM.DevDebugVal
			DBMDebug.Log(Self, "TCC: Setup Event Already Completed for: CheeseMod")
		endIf
	endIf
endEvent	